...
class MlxWarpingSpadeModel:
    ...
    def __init__(self, **kwargs):
        ...
        self.precise_geometry = kwargs.get('precise_geometry', False)
        ...
        if kwargs.get('optimize', False):
            self.spade.prepare_inference()
            self.warping.dense_motion_network.cache_compressed_source = True
        ...
        self._feature_cache_input = None
        self._kp_source_cache_key = None
        self._kp_source_cache_value = None
        self._kp_source_cache_input = None
    ...
    def _feature_to_mx(self, feature):
        ...
        self._feature_cache_input = arr
        return feature_mx

    def _source_kp_to_mx(self, kp_source):
        dtype = mx.float32 if self.precise_geometry else self.dtype
        if isinstance(kp_source, mx.array):
            return kp_source if kp_source.dtype == dtype else kp_source.astype(dtype)
    ...
    def predict(
        ...
        *data,
        ...
        kp_d_mx = mx.array(kp_d).astype(mx.float32) if self.precise_geometry else self._to_mx(kp_d)
