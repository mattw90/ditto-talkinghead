...
def residual_conv(conv, x, *, scaled=False):
    if scaled and x.dtype == mx.float16:
        # MLX's fp16 convolution can overflow partial sums even when its final
        # result is small. Power-of-two scaling keeps those sums representable;
        # add bias only after restoring the original scale.
        out = mx.conv2d(x * (1 / 64), conv.weight, padding=conv.padding,
                        stride=conv.stride, dilation=conv.dilation,
                        groups=conv.groups) * 64
...
class SPADE(nn.Module):
    ...
    def prepare_inference(self):
        if self.mlp_gamma_beta is not None:
            return
        gamma, beta = self.mlp_gamma, self.mlp_beta
        oc, kh, kw, ic = gamma.weight.shape
        fused = nn.Conv2d(ic, 2 * oc, kernel_size=kh, padding=kh // 2)
        fused.weight = mx.concatenate((gamma.weight, beta.weight), axis=0)
        fused.bias = mx.concatenate((gamma.bias, beta.bias), axis=0)
        mx.eval(fused.parameters())
        self.mlp_gamma_beta = fused
    ...
    def __call__(self, x: mx.array, segmap: mx.array) -> mx.array:
        ...
        if self.mlp_gamma_beta is None:
            ...
            gamma, beta = mx.split(self.mlp_gamma_beta(actv), 2, axis=-1)
...
class SPADEResnetBlock(nn.Module):
    ...
    def __call__(self, x, seg):
        # One conditioning resize serves all normalizations in this block.
        if x.shape[1:3] != seg.shape[1:3]:
            seg = _nearest_upsample_nhwc(seg, x.shape[1] // seg.shape[1])
        # Stock fp16 uses fp32 inner blocks to avoid convolution overflow.
        # Prepared inference instead scales fp16 residual convolutions and
        # retains fp32 normalization reductions.
        in_dtype = x.dtype
        upcast = in_dtype == mx.float16 and not self.native_fp16
        ...
        if self.learned_shortcut:
            ...
            x_s = x32
        dx = residual_conv(self.conv_0, nn.leaky_relu(self.norm_0(x32, seg32), 2e-1), scaled=self.native_fp16)
        dx = residual_conv(self.conv_1, nn.leaky_relu(self.norm_1(dx, seg32), 2e-1), scaled=self.native_fp16)
        out = x_s + dx
...
class SPADEDecoder(nn.Module):
    ...
    def prepare_inference(self, *, native_fp16=True):
        for block in [getattr(self, f'G_middle_{i}') for i in range(6)] + [self.up_0, self.up_1]:
            block.native_fp16 = native_fp16
            block.norm_0.prepare_inference()
            block.norm_1.prepare_inference()
            if block.learned_shortcut:
                block.norm_s.prepare_inference()
