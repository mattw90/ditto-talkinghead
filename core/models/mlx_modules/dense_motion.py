...
class DenseMotionNetwork(nn.Module):
    def __init__(
        ...
        self.cache_compressed_source = False
        self._compressed_source_input = None
        self._compressed_source = None
    ...
    def __call__(self, feature, kp_driving, kp_source):
        ...
        # The registered appearance is immutable during a portrait session.
        # Explicitly enabled by that session, never by ordinary model callers.
        source_feature = feature
        if self.cache_compressed_source and self._compressed_source_input is source_feature:
            feature = self._compressed_source
        else:
            feature = self.compress_source(feature)
            if self.cache_compressed_source:
                mx.eval(feature)
                self._compressed_source_input = source_feature
                self._compressed_source = feature
        ...
        # Geometry can stay fp32 while learned convolutions use lower precision.
        x = x.astype(self.compress.weight.dtype)

        prediction = self._hourglass_fn(x)  # (N, D, H, W, hourglass_out)
        return self.predict_motion(prediction, sparse_motion)

    def compress_source(self, feature):
    ...
    def predict_motion(self, prediction, sparse_motion):
